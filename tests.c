...
int main(int argc, char argv[]) {
    ...
    tick();
    is_eql("LDA Imediate sets value to Acc", cpu.acc, 0X26);

    reset();
    setOpByteArg(0XA5, 0X02);
    setMemByte(0X02, 0X27);
    tick();
    is_eql("LDA Zero Page sets value to Acc", cpu.acc, 0X27);
    ...
    cpu.x = 1;
    tick();
    is_eql("LDA Zero Page X sets value to Acc", cpu.acc, 0X28);

    reset();
    setOpWordArg(0XAD, 0X02, 0X21);
    setMemByte(0X2102, 0X29);
    tick();
    is_eql("LDA Absolute sets value to Acc", cpu.acc, 0X29);
    ...
    cpu.x = 1;
    tick();
    is_eql("LDA Absolute X sets value to Acc", cpu.acc, 0X31);
    ...
    cpu.y = 1;
    tick();
    is_eql("LDA Absolute Y sets value to Acc", cpu.acc, 0X32);
    ...
    cpu.x = 1;
    tick();
    is_eql("LDA Ind X sets value to Acc", cpu.acc, 0X11);
    ...
    cpu.y = 0X01;
    tick();
    is_eql("LDA Ind Y without Carry sets value to Acc", cpu.acc, 0X13);
    ...
    cpu.y = 4;
    tick();
    is_eql("LDA Ind Y with Carry sets value to Acc", cpu.acc, 0X13);

    reset();
    setOp(0X4A);
    cpu.acc = 15;
    tick();
    is_eql("4A shifts -> Acc value", cpu.acc, 7);
